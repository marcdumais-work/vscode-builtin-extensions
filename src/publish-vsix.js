...
const { isOnOpenVsxOrg } = require('./version');
...
(async () => {
    ...
    for (const vsix of fs.readdirSync(dist())) {
        ...
        let [bogus, extension, version] = matches;

        // is this extension/version alteady published?
        try {
            let found = await isOnOpenVsxOrg(version, extension);
            if (found) {
                console.log(`Extension ${extension}-${version} is already published - skipping!`)
                continue;
            } 
        } catch (e) {
            console.log('error: ' + e)
